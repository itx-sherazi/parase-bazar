...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await registerUser({ name: email.split("@")[0], email, password, captchaToken });
      router.push("/");
    } catch (err) {
  ...
  return (
    <AuthLayout>
      ...
      <p className="mt-1 text-sm text-zinc-500">Join ParasBazar to start shopping</p>
      ...
      <p className="mt-6 text-center text-sm text-zinc-500">
        ...
        <Link href="/login" className="font-semibold text-amber-500 hover:text-amber-600">
          Sign in
        </Link>
      </p>
    </AuthLayout>
